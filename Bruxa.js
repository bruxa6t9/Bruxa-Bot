...
const watchAndReloadConfig = (dir, type, prop, logName) => {
	...
	fs.watch(dir, (eventType) => {
		if (eventType === type) {
			const oldConfig = global.BruxaBot[prop];
			...
			setTimeout(() => {
				try {
					...
					global.BruxaBot[prop] = JSON.parse(fs.readFileSync(dir, 'utf-8'));
					log.success(logName, `Reloaded ${dir.replace(process.cwd(), "")}`);
				}
				catch (err) {
					log.warn(logName, `Can't reload ${dir.replace(process.cwd(), "")}`);
					global.BruxaBot[prop] = oldConfig;
				}
				finally {
...
global.BruxaBot.envGlobal = global.BruxaBot.configCommands.envGlobal;
global.BruxaBot.envCommands = global.BruxaBot.configCommands.envCommands;
global.BruxaBot.envEvents = global.BruxaBot.configCommands.envEvents;
...
if (config.autoRestart) {
	...
	if (!isNaN(time) && time > 0) {
		utils.log.info("AUTO RESTART", getText("Bruxa", "autoRestart1", utils.convertTime(time, true)));
		setTimeout(() => {
		...
		utils.log.info("AUTO RESTART", getText("Bruxa", "autoRestart2", time));
...
(async () => {
	// ———————————————— CHECK VERSION ———————————————— //
	const { data: { version } } = await axios.get("https://raw.githubusercontent.com/nullshine69/BruxaBot-V1/package.json");
