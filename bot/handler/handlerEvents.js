    ...
    async function onStart() {
      ...
      if (!command) {
        if (!hideNotiMessage.commandNotFound) {
          const { prefixMessages, errorMessages } = require(`${process.cwd()}/languages/getMsgs.js`);
          const prefix = config.prefix;

          if (!commandName) {
            
            const randomIndex = prefixMessages[Math.floor(Math.random() * prefixMessages.length)];
            return await message.reply(randomIndex.replace("${prefix}", prefix));
          }

          // Optimized command suggestion with caching and quick checks
          function getCachedCommandNames() {
            const cmdCount = BruxaBot.commands.size;
            const aliasCount = BruxaBot.aliases.size;
            const cache = BruxaBot._cmdNameCache || {};
            if (!cache.list || cache.cmdCount !== cmdCount || cache.aliasCount !== aliasCount) {
              const list = [...BruxaBot.commands.keys(), ...BruxaBot.aliases.keys()];
              BruxaBot._cmdNameCache = {
                list,
          ...
          function solveCmdName(wrong, right) {
            const randomIndex = errorMessages[Math.floor(Math.random() * errorMessages.length)];

            const finalMsg = randomIndex
              .replace("{wrong}", wrong)
              .replace("{bestMatch}", right);
...
let msg;
          if (bestMatch) {
       msg = solveCmdName(commandName, `${prefix}${bestMatch}`);
          }
