...
function loadScripts(folder, fileName, log, configCommands, api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, getLang, rawCode) {
        ...
        // });
        const storageCommandFilesPath = global.BruxaBot[folder == "cmds" ? "commandFilesPath" : "eventCommandsFilesPath"];
        ...
        try {
                ...
                if (!command.onStart && !command.RA)
                        throw new Error('Function onStart or RA is missing!');
                if (command.RA && typeof command.RA != "function")
                        throw new Error('Function RA must be a function!');
